...
import {
  ...
  ModelPricingSheet,
  type ModelRatioData,
  type ModelPricingEditorPanelRef,
...
type ModelRatioFormPatch = Partial<
  Record<
    | 'ModelPrice'
    | 'ModelRatio'
    | 'CacheRatio'
...
export type ModelRatioVisualEditorRef = {
  commitDraft: () => Promise<ModelRatioFormPatch | boolean>
}
...
export const ModelRatioVisualEditor = memo(
  forwardRef<ModelRatioVisualEditorRef, ModelRatioVisualEditorProps>(
    function ModelRatioVisualEditor(
      ...
      const editorPanelRef = useRef<ModelPricingEditorPanelRef>(null)
      ...
      useImperativeHandle(
        ...
        () => ({
          commitDraft: () => {
            if (!editorOpen) return Promise.resolve(true)
            return (
              editorPanelRef.current?.submitDraft() ?? Promise.resolve(true)
            )
          },
        }),
        [editorOpen]
      ...
      const handleDelete = useCallback(
        (name: string) => {
          ...
          const patch: ModelRatioFormPatch = {
            ...
            <div className='hidden min-w-0 md:block'>
              {editorOpen ? (
                <ModelPricingEditorPanel
                  ref={editorPanelRef}
                  onSave={handleSave}
                  onDraftChange={persistPricingData}
